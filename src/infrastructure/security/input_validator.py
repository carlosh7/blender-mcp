...
class InputValidator:
    ...
    def validate_string(
        self, value: str, field_name: str = "input", rules: list[ValidationRule] | None = None
    ) -> str:
        """
    ...
    def validate_filename(self, filename: str, field_name: str = "filename") -> str:
        ...
        # Check length
        if len(filename) > 255:
            raise InputValidationError(f"{field_name}: Filename too long (max 255 chars)")
        ...
        if name_without_ext in reserved:
            raise InputValidationError(f"{field_name}: Reserved filename: {filename}")
    ...
    def validate_url(self, url: str, field_name: str = "url") -> str:
        ...
        if not url_pattern.match(url):
            raise InputValidationError(f"{field_name}: Invalid URL format")

        return url

    def validate_code(self, code: str, field_name: str = "code") -> str:
        """
        Validate code input.
        ...
        for pattern, message in dangerous_patterns:
            if re.search(pattern, code, re.IGNORECASE):
                raise InputValidationError(f"{field_name}: {message}")
    ...
    def validate_integer(
        ...
        if min_value is not None and int_value < min_value:
            raise InputValidationError(f"{field_name}: Value {int_value} below minimum {min_value}")

        if max_value is not None and int_value > max_value:
            raise InputValidationError(f"{field_name}: Value {int_value} above maximum {max_value}")
    ...
    def validate_float(
        ...
        try:
            ...
            raise InputValidationError(f"{field_name}: Expected float, got {type(value).__name__}")
    ...
    def validate_enum(self, value: Any, allowed_values: set[Any], field_name: str = "input") -> Any:
        """
        Validate enum input.
    ...
    def _check_injection(self, value: str, field_name: str) -> None:
        ...
        for pattern in self.injection_patterns:
            if pattern.search(value):
                raise InputValidationError(f"{field_name}: Potential injection attempt detected")
    ...
    def _check_path_traversal(self, value: str, field_name: str) -> None:
        ...
        for pattern in self.path_patterns:
            if pattern.search(value):
                raise InputValidationError(f"{field_name}: Potential path traversal detected")

    def _apply_rule(self, value: str, field_name: str, rule: ValidationRule) -> None:
        """Apply validation rule."""
        if rule.pattern and not re.match(rule.pattern, value):
            raise InputValidationError(f"{field_name}: {rule.name} validation failed")

        if rule.min_length and len(value) < rule.min_length:
            raise InputValidationError(f"{field_name}: Minimum length {rule.min_length} not met")

        if rule.max_length and len(value) > rule.max_length:
            raise InputValidationError(f"{field_name}: Maximum length {rule.max_length} exceeded")

        if rule.blocked_chars:
            for char in rule.blocked_chars:
                if char in value:
                    raise InputValidationError(f"{field_name}: Blocked character '{char}' found")

        if rule.custom_validator and not rule.custom_validator(value):
            raise InputValidationError(f"{field_name}: Custom validation failed")
