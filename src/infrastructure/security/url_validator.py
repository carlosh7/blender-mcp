...
class URLValidator:
    ...
    def __init__(self, custom_allowed: set[str] | None = None):
        """
    ...
    def validate(self, url: str) -> URLValidationResult:
        ...
        try:
            ...
            return URLValidationResult(is_safe=False, errors=errors, warnings=warnings)
        ...
        return URLValidationResult(is_safe=is_safe, errors=errors, warnings=warnings)
    ...
    def validate_strict(self, url: str) -> None:
        ...
        if not result.is_safe:
            raise URLError(f"URL validation failed: {'; '.join(result.errors)}")
