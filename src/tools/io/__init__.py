...
def _try_export(export_func, filepath: str, **kwargs) -> Dict:
    ...
    try:
        ...
        if hasattr(export_func, 'poll') and not export_func.poll():
            return {"error": f"Export needs GUI (background mode). Save .blend and export manually."}
...
def export_obj(filepath: str, use_selection: bool = False) -> Dict:
    try:
        ...
        if hasattr(bpy.ops.wm, 'obj_export'):
            return _try_export(bpy.ops.wm.obj_export, filepath, export_selected_objects=use_selection)
