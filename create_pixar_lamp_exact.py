...
# Materials Setup
mat_metal_gray = pbr_factory.create_pbr_metal(
    "MAT_Luxo_GrayMetal", color=(0.75, 0.77, 0.80), brushed=True
)
...
def create_bar_between_points(
    name, p_a, p_b, y_offset, width=0.014, thickness=0.004, material=mat_metal_gray
):
...
for offset_y in [-0.03, 0.03]:
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.006, depth=0.008, location=(p0[0], offset_y, p0[2]), rotation=(math.pi / 2, 0, 0)
    )
...
bpy.ops.mesh.primitive_cylinder_add(
    radius=0.018, depth=0.04, location=p2, rotation=(math.pi / 2, 0, 0)
)
...
bpy.ops.mesh.primitive_cylinder_add(
    radius=0.007, depth=0.018, location=(p2[0] - 0.015, 0, p2[2] + 0.045)
)
...
for s_idx in range(4):
    bpy.ops.mesh.primitive_cube_add(
        size=1.0, location=(p2[0] + 0.01 + s_idx * 0.008, 0, p2[2] + 0.025)
    )
...
bulb_pos = (p2[0] + 0.075 * shade_dir_x, 0.0, p2[2] + 0.075 * shade_dir_z)
...
sp_end_l = (
    p0[0] + 0.01 + 0.18 * math.sin(theta1_rad),
    -0.026,
    p0[2] + 0.02 + 0.18 * math.cos(theta1_rad),
)
