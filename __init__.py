try:
    ...
    # Importado fuera de Blender (p. ej. pytest subiendo por la cadena de
    # paquetes hasta este __init__.py): el addon no puede operar sin bpy.
    bpy = None
...
def register():
    ...
    try:
        ...
        print(
            f"[AXIOM] {'✅' if s_9876 else '❌'} PORT 9876 (SOCKET): {'OPERATIVO' if s_9876 else 'DESCONECTADO'}"
        )
        print(
            f"[AXIOM] {'✅' if s_9877 else '❌'} PORT 9877 (HTTP): {'OPERATIVO' if s_9877 else 'DESCONECTADO'}"
        )
        print(
            f"[AXIOM] {'✅' if s_9879 else '❌'} PORT 9879 (MCP): {'OPERATIVO' if s_9879 else 'DESCONECTADO'}"
        )
        ...
        classes = [
            chat_types.ChatMsg,
            chat_types.ModelItem,
            panels_chat.PN_PT_Chat,
            panels_config.PN_PT_Config,
            props.ChatData,
            props.ModelsData,
            props.MCP_UL_Chat,
            panels_chat.BLENDERMCP_OT_OpenWeb,
        ]
        for p in panels_integrations.PANELS:
            classes.append(p)
        for cls in classes:
            try:
