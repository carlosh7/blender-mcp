...
def is_ui_required(message):
    """Perintah UI-only gagal anggun di background; hitung sebagai SKIP."""
    return any(token in str(message) for token in
               ("memerlukan antarmuka", "window 3D", "Tidak ada window"))
...
COMMANDS = [
    ...
    # Batch (paling akhir: operasi ini mengubah nama/menghapus objek)
...
def is_ui_required(message):
    return any(t in str(message) for t in
               ("memerlukan antarmuka", "window 3D", "Tidak ada window"))
if not srv.running:
    srv.start()
...
results = {{"pass": 0, "skip": 0, "fail": 0, "total": len(commands), "details": []}}
...
for cmd_type, params, category in commands:
    try:
        ...
        elif is_ui_required(r.get("message", "")):
            results["skip"] += 1
            results["details"].append({{"cmd": cmd_type, "status": "SKIP (UI)", "category": category}})
        else:
            results["fail"] += 1
...
def run_in_blender():
    """Run the full command surface against `blender -b` and print results."""
    import subprocess
    result = subprocess.run(
        [BLENDER, "-b", "--python-expr", SCRIPT],
        capture_output=True, text=True, timeout=60
    )
    return _report(result.stdout)


def _report(output):
    if "---RESULTS---" not in output:
        return f"ERROR: Hasil tidak ditemukan\n{output[:1000]}"
    ...
    lines = [f"\n{'='*50}",
             "  VALIDASI TOOLS",
             f"{'='*50}",
             f"  Total: {r['total']} | {PASS} {r['pass']} | {SKIP} {r.get('skip', 0)} | {FAIL} {r['fail']}",
             f"{'='*50}"]
    cats = {}
    for d in r["details"]:
        cat = d["category"]
        if cat not in cats:
            cats[cat] = {"pass": 0, "skip": 0, "fail": 0, "total": 0}
        cats[cat]["total"] += 1
        if d["status"] == "PASS":
            cats[cat]["pass"] += 1
        elif d["status"] == "SKIP (UI)":
            cats[cat]["skip"] += 1
        else:
            cats[cat]["fail"] += 1
    for cat, stats in sorted(cats.items()):
        icon = PASS if stats["fail"] == 0 else FAIL
        lines.append(f"  {icon} {cat}: {stats['pass']}/{stats['total']}")
    lines.append("")
    if r["fail"] > 0:
        lines.append(f"  {FAIL} GAGAL:")
        for d in r["details"]:
            if d["status"] not in ("PASS", "SKIP (UI)"):
                lines.append(f"    - {d['cmd']}: {d['status']}")
    else:
        lines.append(f"  {PASS} SEMUA TOOLS BERFUNGSI")
    lines.append(f"{'='*50}")
    return "\n".join(lines)


if __name__ == "__main__":
    print(run_in_blender())
