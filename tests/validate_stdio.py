...
def test(name, input_data, check_fn):
    try:
        r = subprocess.run(BRIDGE, input=input_data, capture_output=True, text=True, timeout=10)
...
test(
    "tools/call",
    '{"method":"tools/call","id":3,"params":{"name":"get_scene_info","arguments":{}}}\n',
    lambda r: "result" in r,
)

print(f"\n{'=' * 50}")
print("  VALIDACION STDIO BRIDGE")
print(f"{'=' * 50}")
