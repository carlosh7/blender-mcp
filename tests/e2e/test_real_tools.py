...
class TestRealObjectTools:
    ...
    def test_object_create_cube(self):
        result = call_tool("object.create", {"type": "MESH", "name": "RealCube", "location": [0, 0, 0]})
        assert result is not None
        assert result.get("success") or result.get("name")
    
    @skip_without_blender
    def test_object_create_sphere(self):
        result = call_tool("object.create", {"type": "MESH", "name": "RealSphere", "location": [3, 0, 0]})
        assert result is not None
    
    @skip_without_blender
    def test_object_create_cylinder(self):
        result = call_tool("object.create", {"type": "MESH", "name": "RealCylinder", "location": [-3, 0, 0]})
        assert result is not None
    
    @skip_without_blender
    def test_object_create_cone(self):
        result = call_tool("object.create", {"type": "MESH", "name": "RealCone", "location": [0, 3, 0]})
        assert result is not None
    
    @skip_without_blender
    def test_object_create_torus(self):
        result = call_tool("object.create", {"type": "MESH", "name": "RealTorus", "location": [0, -3, 0]})
        assert result is not None
    
    @skip_without_blender
    def test_object_create_light(self):
        result = call_tool("object.create", {"type": "LIGHT", "name": "RealLight", "location": [5, 5, 5]})
        assert result is not None
    
    @skip_without_blender
    def test_object_create_camera(self):
        result = call_tool("object.create", {"type": "CAMERA", "name": "RealCamera", "location": [0, -5, 2]})
        assert result is not None
...
class TestRealMaterialTools:
    ...
    def test_material_create(self):
        result = call_tool("material.create", {"name": "RealMat", "color": [1, 0, 0, 1], "metallic": 0.5})
        assert result is not None
    
    @skip_without_blender
    def test_material_assign(self):
        call_tool("material.create", {"name": "AssignMat", "color": [0, 1, 0, 1]})
        result = call_tool("material.assign", {"object_name": "RealCube", "material_name": "AssignMat"})
        assert result is not None
...
class TestRealLightTools:
    ...
    def test_light_create_area(self):
        result = call_tool("light.create", {"type": "AREA", "name": "RealAreaLight", "location": [3, -3, 5], "energy": 1000})
        assert result is not None
    
    @skip_without_blender
    def test_light_create_point(self):
        result = call_tool("light.create", {"type": "POINT", "name": "RealPointLight", "location": [0, 0, 3]})
        assert result is not None
    
    @skip_without_blender
    def test_light_create_sun(self):
        result = call_tool("light.create", {"type": "SUN", "name": "RealSunLight", "location": [0, 0, 10]})
        assert result is not None
    
    @skip_without_blender
    def test_light_create_spot(self):
        result = call_tool("light.create", {"type": "SPOT", "name": "RealSpotLight", "location": [0, -5, 3]})
        assert result is not None
...
class TestRealAnimationTools:
    ...
    def test_animation_set_keyframe(self):
        result = call_tool("animation.set_keyframe", {"object_name": "RealCube", "property": "location", "frame": 1, "value": [0, 0, 0]})
        assert result is not None
    
    @skip_without_blender
    def test_animation_set_keyframe2(self):
        result = call_tool("animation.set_keyframe", {"object_name": "RealCube", "property": "location", "frame": 50, "value": [5, 0, 3]})
        assert result is not None
...
class TestRealShaderNodeTools:
    ...
    def test_shader_add_node(self):
        result = call_tool("shader.add_node", {"material_name": "RealMat", "node_type": "ShaderNodeTexNoise"})
        assert result is not None
...
class TestRealBatchTools:
    ...
    def test_batch_turntable(self):
        result = call_tool("batch.turntable", {"object_name": "RealSphere", "frames": 30, "axis": "Z"})
        assert result is not None
