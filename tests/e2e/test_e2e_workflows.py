...
class TestE2EProductVisualization:
    ...
    def test_product_scene_creation(self):
        ...
        # 1. Create product
        result = call_tool(
            "object.create", {"type": "MESH", "name": "Product", "location": [0, 0, 0]}
        )
        assert result is not None

        # 2. Add modifiers
        result = call_tool("modifier.add", {"object_name": "Product", "type": "SUBSURF"})
        assert result is not None

        # 3. Create material
        result = call_tool(
            "material.create", {"name": "ProductMat", "color": [0.8, 0.2, 0.2, 1], "metallic": 0.5}
        )
        assert result is not None

        # 4. Assign material
        result = call_tool(
            "material.assign", {"object_name": "Product", "material_name": "ProductMat"}
        )
        assert result is not None
...
class TestE2ECharacterRig:
    ...
    def test_character_rig_creation(self):
        ...
        # 1. Create armature
        result = call_tool(
            "rigging.create_armature", {"name": "CharacterArm", "location": [0, 0, 0]}
        )
        assert result is not None

        # 2. Create character mesh
        result = call_tool(
            "object.create", {"type": "MESH", "name": "CharacterMesh", "location": [0, 0, 0]}
        )
        assert result is not None

        # 3. Create vertex group
        result = call_tool(
            "rigging.create_vertex_group", {"object_name": "CharacterMesh", "name": "Body"}
        )
        assert result is not None

        # 4. Setup camera for rigging
        result = call_tool("camera.create", {"name": "RigCam", "location": [0, -5, 2], "lens": 50})
        assert result is not None
...
class TestE2EAnimation:
    ...
    def test_animation_workflow(self):
        ...
        # 1. Create object
        result = call_tool(
            "object.create", {"type": "MESH", "name": "AnimObj", "location": [0, 0, 0]}
        )
        assert result is not None
        ...
        result = call_tool(
            ...
            {"object_name": "AnimObj", "property": "location", "frame": 1, "value": [0, 0, 0]},
        )
        assert result is not None

        result = call_tool(
            "animation.set_keyframe",
            {"object_name": "AnimObj", "property": "location", "frame": 50, "value": [5, 0, 3]},
        )
        assert result is not None

        # 3. Set interpolation
        result = call_tool(
            "animation.set_interpolation", {"object_name": "AnimObj", "interpolation": "BEZIER"}
        )
        assert result is not None

        # 4. Get F-curves
        result = call_tool("animation.get_fcurves", {"object_name": "AnimObj"})
        assert result is not None
...
class TestE2EGeometryNodes:
    ...
    def test_geometry_nodes_workflow(self):
        ...
        # 1. Create object
        result = call_tool(
            "object.create", {"type": "MESH", "name": "GNObj", "location": [0, 0, 0]}
        )
        assert result is not None

        # 2. Create node group
        result = call_tool("geonodes.create_group", {"name": "ProceduralSetup"})
        assert result is not None
...
class TestE2EShaderNodes:
    ...
    def test_shader_workflow(self):
        ...
        # 1. Create material
        result = call_tool("material.create", {"name": "ShaderMat", "color": [1, 0, 0, 1]})
        assert result is not None

        # 2. Add noise node
        result = call_tool(
            "shader.add_node", {"material_name": "ShaderMat", "node_type": "ShaderNodeTexNoise"}
        )
        assert result is not None

        # 3. List nodes
        result = call_tool("shader.list_nodes", {"material_name": "ShaderMat"})
        assert result is not None
...
class TestE2ESceneManagement:
    ...
    def test_scene_management(self):
        ...
        for i in range(5):
            result = call_tool(
                "object.create", {"type": "MESH", "name": f"Obj_{i}", "location": [i * 2, 0, 0]}
            )
            assert result is not None
        ...
        # 4. Mesh analysis
        result = call_tool("scene_utils.mesh_analysis", {"object_name": "Obj_0"})
        assert result is not None
...
class TestE2EBatchOperations:
    ...
    def test_batch_workflow(self):
        ...
        for i in range(3):
            call_tool(
                "object.create", {"type": "MESH", "name": f"Batch_{i}", "location": [i * 2, 0, 0]}
            )

        # 2. Batch rename
        result = call_tool("batch.rename", {"pattern": "Batch", "replace": "Item"})
        assert result is not None
...
class TestE2EExport:
    ...
    def test_export_workflow(self):
        ...
        # 1. Create object
        call_tool("object.create", {"type": "MESH", "name": "ExportObj", "location": [0, 0, 0]})

        # 2. Save blend file
        result = call_tool("io.save_file", {"filepath": "/tmp/e2e_test.blend"})
        assert result is not None
...
class TestE2EExecuteCode:
    ...
    def test_code_execution(self):
        ...
        result = send_mcp_request(
            "tools/call", {"name": "scene.get_info", "arguments": {"include_objects": True}}
        )
        assert result is not None
