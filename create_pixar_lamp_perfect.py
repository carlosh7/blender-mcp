...
# Materials Setup
mat_gray = pbr_factory.create_pbr_metal('MAT_Luxo_GrayMetal', color=(0.72, 0.74, 0.77), brushed=True)
...
base_profile = [
    (0.001, 0.0), (0.12, 0.0), (0.125, 0.005), (0.12, 0.015),
    (0.09, 0.025), (0.03, 0.035), (0.022, 0.05), (0.026, 0.055),
    (0.026, 0.065), (0.018, 0.07), (0.001, 0.07)
]
...
for offset_y in [-0.024, 0.024]:
    bpy.ops.mesh.primitive_cylinder_add(radius=0.005, depth=0.006, location=(p0[0], offset_y, p0[2]), rotation=(math.pi/2, 0, 0))
...
for i, offset_y in enumerate([-0.018, 0.018]):
    create_arm_bar(f'GEO_Luxo_LowerBar_{i}', l1, location=(p0[0], offset_y, p0[2]), rotation_y=theta1_rad)
...
for i, offset_y in enumerate([-0.015, 0.015]):
    create_arm_bar(f'GEO_Luxo_UpperBar_{i}', l2, location=(p1[0], offset_y, p1[2]), rotation_y=theta2_rad)
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.014, depth=0.035, location=p2, rotation=(math.pi/2, 0, 0))
...
shade_profile = [
    (0.018, 0.0), (0.032, 0.0), (0.034, 0.045), (0.042, 0.06),
    (0.06, 0.09), (0.072, 0.12), (0.076, 0.145), (0.074, 0.15),
    (0.068, 0.146), (0.055, 0.118), (0.038, 0.058), (0.03, 0.042),
    (0.03, 0.005), (0.014, 0.0)
]
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.006, depth=0.014, location=(p2[0] - 0.01, 0, p2[2] + 0.032))
...
for s_idx in range(4):
    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(p2[0] + 0.006 + s_idx*0.006, 0, p2[2] + 0.018))
...
bulb_pos = (
    p2[0] + 0.05 * shade_dir_x,
    0.0,
    p2[2] + 0.05 * shade_dir_z
)
...
sp_end_l = (p0[0] + 0.008 + 0.14*math.sin(theta1_rad), -0.020, p0[2] + 0.015 + 0.14*math.cos(theta1_rad))
create_aligned_spring('GEO_Luxo_Spring_Lower_L', sp_start_l, sp_end_l)

sp_start_r = (p0[0] + 0.008, 0.020, p0[2] + 0.015)
sp_end_r = (p0[0] + 0.008 + 0.14*math.sin(theta1_rad), 0.020, p0[2] + 0.015 + 0.14*math.cos(theta1_rad))
