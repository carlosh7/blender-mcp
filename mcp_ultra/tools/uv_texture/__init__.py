...
TOOLS = [
    ...
    Tool(
        "texture.list", ToolCategory.UV_TEXTURE, "List all textures", ToolPermission.READ_ONLY, {}
    ),
    Tool(
...
def unwrap(method: str = "ANGLE_BASED", margin: float = 0.001) -> dict:
    try:
        ...
        if method == "SMART":
            bpy.ops.uv.smart_project(angle_limit=66, margin=margin)
        else:
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                "note": "UV operation completed (background mode)",
            }
    except Exception as e:
        return {"error": str(e)}
...
def delete_uv(object_name: str, name: str) -> dict:
    try:
        ...
        if not uv:
            return {"error": f"UV map not found: {name}"}
...
def create_texture(
    name: str = "Texture", width: int = 1024, height: int = 1024, color: tuple = (0.5, 0.5, 0.5, 1)
) -> dict:
    try:
        import bpy
...
def assign_to_material(material_name: str, texture_name: str, slot: str = "Base Color") -> dict:
    try:
        ...
        if not bsdf:
            return {"error": "No Principled BSDF node"}
...
HANDLERS = {
    "uv.unwrap": unwrap,
    "uv.pack": pack,
    "uv.smart_project": smart_project,
    "uv.list": list_uv,
    "uv.create": create_uv,
    "uv.delete": delete_uv,
    "texture.create": create_texture,
    "texture.list": list_textures,
    "texture.assign_to_material": assign_to_material,
    "uv.bake": bake,
}
