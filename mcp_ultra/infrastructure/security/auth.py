...
class Authenticator:
    ...
    def generate_token(
        self, user_id: str, permissions: list = None, expires_in: int = 86400
    ) -> str:
        """
    ...
    def get_user_tokens(self, user_id: str) -> list:
        ...
        """
        return [token for token in self._tokens.values() if token.user_id == user_id]
...
def validate_token(token: str) -> AuthToken | None:
