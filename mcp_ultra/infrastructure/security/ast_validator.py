...
class ASTValidator:
    ...
    def _validate_allowlist(self, code: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(is_safe=False, errors=[f"Syntax error: {e}"], warnings=[])
        ...
        return ValidationResult(is_safe=len(errors) == 0, errors=errors, warnings=warnings)
    ...
    def validate_strict(self, code: str) -> None:
        ...
        if not result.is_safe:
            raise SecurityError(f"Code validation failed: {'; '.join(result.errors)}")
...
_validator: ASTValidator | None = None
