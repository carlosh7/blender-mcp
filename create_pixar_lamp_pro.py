...
# Materials Setup
mat_enamel = pbr_factory.create_pbr_metal('MAT_Luxo_EnamelGray', color=(0.78, 0.80, 0.83), brushed=True)
mat_chrome = pbr_factory.create_pbr_metal('MAT_Luxo_Chrome', color=(0.88, 0.89, 0.92), brushed=True)
mat_dark_steel = pbr_factory.create_pbr_metal('MAT_Luxo_DarkSteel', color=(0.18, 0.20, 0.22), brushed=True)
...
# A. STEPPED DISK BASE
base_profile = [
    (0.001, 0.0), (0.17, 0.0), (0.18, 0.006), (0.178, 0.018),
    (0.14, 0.030), (0.06, 0.040), (0.04, 0.048), (0.04, 0.058),
    (0.048, 0.064), (0.048, 0.075), (0.038, 0.080), (0.038, 0.092),
    (0.024, 0.098), (0.001, 0.098)
]
...
for offset_y in [-0.028, 0.028]:
    bpy.ops.mesh.primitive_cylinder_add(radius=0.006, depth=0.008, location=(0, offset_y, 0.125), rotation=(math.pi/2, 0, 0))
...
p1 = (
    ...
    0.0,
    p0[2] + 0.027 + l1 * math.cos(theta1_rad)
)
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.016, depth=0.04, location=p2, rotation=(math.pi/2, 0, 0))
...
shade_profile = [
    (0.001, 0.0), (0.02, 0.005), (0.038, 0.018), (0.042, 0.065),
    (0.05, 0.085), (0.07, 0.115), (0.098, 0.155), (0.125, 0.19),
    (0.132, 0.20), (0.128, 0.205), (0.12, 0.20), (0.09, 0.15),
    (0.06, 0.11), (0.044, 0.08), (0.036, 0.06), (0.034, 0.02),
    (0.015, 0.005), (0.001, 0.005)
]
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.007, depth=0.016, location=(p2[0] - 0.012, 0, p2[2] + 0.036))
...
bulb_pos = (
    p2[0] + 0.065 * shade_dir_x,
    0.0,
    p2[2] + 0.065 * shade_dir_z
)
...
sp_end_l = (p0_pivot[0] + 0.006 + 0.16*math.sin(theta1_rad), -0.024, p0_pivot[2] + 0.012 + 0.16*math.cos(theta1_rad))
create_aligned_spring('GEO_Luxo_Spring_Lower_L', sp_start_l, sp_end_l)

sp_start_r = (p0_pivot[0] + 0.006, 0.024, p0_pivot[2] + 0.012)
sp_end_r = (p0_pivot[0] + 0.006 + 0.16*math.sin(theta1_rad), 0.024, p0_pivot[2] + 0.012 + 0.16*math.cos(theta1_rad))
