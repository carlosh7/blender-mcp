...
# Materials Setup
mat_matte_plastic = pbr_factory.create_pbr_metal(
    "MAT_Bose_BlackPlastic", color=(0.08, 0.09, 0.10), brushed=False
)
mat_dark_grille = pbr_factory.create_pbr_metal(
    "MAT_Bose_DarkGrille", color=(0.14, 0.15, 0.16), brushed=True
)
mat_chrome_logo = pbr_factory.create_pbr_metal(
    "MAT_Bose_ChromeLogo", color=(0.90, 0.91, 0.93), brushed=True
)
mat_speaker_cone = pbr_factory.create_pbr_metal(
    "MAT_Bose_SpeakerCone", color=(0.18, 0.19, 0.20), brushed=False
)
mat_io_panel = pbr_factory.create_pbr_metal(
    "MAT_Bose_IOPanel", color=(0.05, 0.05, 0.06), brushed=True
)
...
for s_idx in range(8):
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.025, depth=0.015, location=(0, -0.068, sz), rotation=(math.pi / 2, angle_offset, 0)
    )
...
for w_z in [sub_z - 0.08, sub_z + 0.08]:
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.085, depth=0.03, location=(sub_x, sub_y - 0.20, w_z), rotation=(math.pi / 2, 0, 0)
    )
