...
def add_object_to_plan(
    object_type, position, parent=None, anchor=None, collection=None, material=None
):
    """
...
def execute_plan(plan=None):
    ...
    for i, obj in enumerate(_scene_plan["execution_order"], 1):
        ...
        try:
            # Crear objeto
            created = creation_rules.create_object(obj_type, position, collection, material)
            ...
            # Validar
            validation = validator.validate_object(list(created.values())[0].name)
...
def load_template(template_name):
    ...
    if template_name not in SCENE_TEMPLATES:
        raise ValueError(
            f"Plantilla no encontrada: {template_name}. Disponibles: {list(SCENE_TEMPLATES.keys())}"
        )
