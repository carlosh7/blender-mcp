...
def parse_description_manual(description):
    ...
    color_map = {
        ...
        "verde": (0.1, 0.7, 0.1),
        "yellow": (0.9, 0.9, 0.1),
        "amarillo": (0.9, 0.9, 0.1),
        "black": (0.05, 0.05, 0.05),
        "negro": (0.05, 0.05, 0.05),
        "white": (0.9, 0.9, 0.9),
        "blanco": (0.9, 0.9, 0.9),
        "gold": (0.85, 0.65, 0.1),
        "dorado": (0.85, 0.65, 0.1),
        "silver": (0.8, 0.8, 0.8),
        "plateado": (0.8, 0.8, 0.8),
    }
...
def generate_3d_from_parsed(parsed):
    ...
    parsed.get("shape", "cube")
    parsed.get("color", (0.5, 0.5, 0.5))
    parsed.get("size", 1.0)
    parsed.get("material", "plastic")
    ...
    for x in [-0.18, 0.18]:
        for y in [-0.18, 0.18]:
            bpy.ops.mesh.primitive_cylinder_add(
                radius=size * 0.02, depth=size * 0.45, location=(x * size, y * size, size * 0.22)
            )
...
def _create_vehicle(parsed):
    ...
    for x in [-0.6, 0.6]:
        for y in [-0.4, 0.4]:
            bpy.ops.mesh.primitive_cylinder_add(
                radius=size * 0.12, depth=size * 0.08, location=(x * size, y * size, size * 0.12)
            )
...
def _create_character(parsed):
    ...
    # Cabeza
    bpy.ops.mesh.primitive_uv_sphere_add(radius=size * 0.1, location=(0, 0, size * 0.85))
    ...
    for side in ["L", "R"]:
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            radius=size * 0.03, depth=size * 0.3, location=(x_sign * size * 0.22, 0, size * 0.65)
        )
    ...
    for side in ["L", "R"]:
        x_sign = 1 if side == "L" else -1
        bpy.ops.mesh.primitive_cylinder_add(
            radius=size * 0.04, depth=size * 0.35, location=(x_sign * size * 0.08, 0, size * 0.15)
        )
        leg = bpy.context.active_object
...
def _create_animal(parsed):
    ...
    # Cabeza
    bpy.ops.mesh.primitive_uv_sphere_add(radius=size * 0.12, location=(0, size * 0.4, size * 0.5))
    head = bpy.context.active_object
    ...
    for i, pos in enumerate(leg_positions):
        bpy.ops.mesh.primitive_cylinder_add(radius=size * 0.025, depth=size * 0.3, location=pos)
        leg = bpy.context.active_object
