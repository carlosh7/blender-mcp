...
def create_sculpt_base(primitive_type="sphere", subdivisions=4):
    ...
    if primitive_type == "sphere":
        bpy.ops.mesh.primitive_uv_sphere_add(
            radius=1, segments=64, ring_count=32, location=(0, 0, 0)
        )
        ...
        raise ValueError(f"Tipo de primitiva no soportada: {primitive_type}")
...
def apply_multiresolution_sculpt(obj, levels=3):
    ...
    obj.modifiers.get("Multires") or obj.modifiers.new("Multires", "MULTIRES")
...
def smooth_entire_mesh(obj, iterations=5):
...
def create_face_base(head_radius=0.12):
    ...
    # Cabeza base (esfera)
    bpy.ops.mesh.primitive_uv_sphere_add(
        radius=head_radius, segments=32, ring_count=16, location=(0, 0, 0)
    )
...
def add_eyes(parent, spacing=0.04, size=0.015):
    ...
    for side in ["L", "R"]:
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(radius=size, location=(x_sign * spacing, -0.06, 0.02))
...
def create_limbs(side="L", limb_type="arm"):
    ...
    if limb_type == "arm":
        # Hombro
        bpy.ops.mesh.primitive_uv_sphere_add(radius=0.03, location=(x_sign * 0.15, 0, 0.4))
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.025, depth=0.25, location=(x_sign * 0.2, 0, 0.3)
        )
        ...
        # Brazo inferior
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.02, depth=0.22, location=(x_sign * 0.25, 0, 0.15)
        )
        ...
        # Muslo
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.04, depth=0.3, location=(x_sign * 0.08, 0, -0.2)
        )
        upper = bpy.context.active_object
        ...
        # Pantorrilla
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.03, depth=0.28, location=(x_sign * 0.08, 0, -0.5)
        )
        lower = bpy.context.active_object
        ...
        # Pie
        bpy.ops.mesh.primitive_cube_add(size=1, location=(x_sign * 0.08, 0.03, -0.65))
...
def sculpt_face(head_obj):
    ...
    smooth_entire_mesh(head_obj, iterations=3)

    print("Cara sculpted: nariz + ojos + boca")
...
def sculpt_body(torso_obj):
    ...
    # Suavizar torso
    smooth_entire_mesh(torso_obj, iterations=3)

    print("Cuerpo sculpted: torso + 4 extremidades")
    return {
        "torso": torso_obj,
        "left_arm": left_arm,
        "right_arm": right_arm,
