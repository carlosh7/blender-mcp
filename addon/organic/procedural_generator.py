...
def procedural_tree(trunk_height=2.0, crown_radius=1.5, branches=8):
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.15, depth=trunk_height, location=(0, 0, trunk_height / 2)
    )
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        radius=crown_radius, location=(0, 0, trunk_height + crown_radius * 0.7)
    )
...
def procedural_house(rooms=4, floors=1, width=10, depth=8):
    ...
    bpy.ops.mesh.primitive_cone_add(
        radius1=width * 0.7, radius2=0, depth=2, vertices=4, location=(0, 0, 4)
    )
...
def procedural_vehicle(body_type="sedan", length=4.5, width=1.8):
    ...
    for x in [-length * 0.35, length * 0.35]:
        for y in [-width * 0.4, width * 0.4]:
            ...
            wheel.data.materials.append(
                bpy.data.materials.get("Rubber") or create_rubber_material()
            )
