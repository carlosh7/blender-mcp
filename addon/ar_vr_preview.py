...
def export_for_ar_vr(target: str, filepath: str | None = None) -> dict:
    """
    ...
    try:
        ...
        if config["extension"] == ".glb":
            bpy.ops.export_scene.gltf(filepath=filepath, **config["options"])
        elif config["extension"] == ".fbx":
            bpy.ops.export_scene.fbx(filepath=filepath, **config["options"])
