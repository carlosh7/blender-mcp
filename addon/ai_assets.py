...
def _meshy_text_to_3d(description):
    ...
    try:
        ...
        payload = {"mode": "preview", "prompt": description, "art_style": "realistic"}
...
def _download_and_import(model_url, save_dir=str(_temp_dir("assets"))):
    ...
    Args:
        model_url: URL del archivo GLB
        save_dir: Directorio de descarga

    Returns:
        Objeto importado o None
    """
    if bpy is None:
        return None
    try:
        os.makedirs(save_dir, exist_ok=True)
        filepath = os.path.join(save_dir, f"ai_model_{int(time.time())}.glb")
        urllib.request.urlretrieve(model_url, filepath)

        before = set(bpy.data.objects)
        bpy.ops.import_scene.gltf(filepath=filepath)
        new_objects = [o for o in bpy.data.objects if o not in before]
        print(f"Modelo IA importado: {len(new_objects)} objetos desde {filepath}")
        return new_objects[0] if new_objects else None
    except Exception as e:
        print(f"Error descargando/importando modelo IA: {e}")
        return None
