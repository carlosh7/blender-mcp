...
from bpy.props import StringProperty, CollectionProperty, IntProperty, EnumProperty, FloatProperty, BoolProperty
...
def save_history(messages):
    os.makedirs(CONFIG_DIR, exist_ok=True)
    data = [{"role": m.role, "content": m.content, "timestamp": m.timestamp} for m in messages[-100:]]
...
def fetch_ollama_models(url):
    ...
    for m in result.get("models", []):
        models.append({"name": m["name"], "is_free": True, "cost_in": 0, "cost_out": 0, "context_length": 0})
    return models, None
...
def try_local(msg, props):
    ...
    if any(w in m for w in ["create", "make", "add", "crear", "hacer"]):
        if "chair" in m or "silla" in m:
            return "🪑 Creating chair...", """bpy.ops.mesh.primitive_cube_add(size=1, location=(0,0,0.45)); s=bpy.context.active_object; s.name="Chair"; s.scale=(0.45,0.45,0.04); mat=bpy.data.materials.new("Wood"); mat.use_nodes=True; b=mat.node_tree.nodes.get("Principled BSDF"); b.inputs["Base Color"].default_value=(0.4,0.25,0.12,1); b.inputs["Roughness"].default_value=0.7; s.data.materials.append(mat)
for x in [-0.18,0.18]:
 for y in [-0.18,0.18]:
...
print("Chair created")"""
            ...
            return "🪑 Creating table...", """bpy.ops.mesh.primitive_cube_add(size=1, location=(0,0,0.75)); t=bpy.context.active_object; t.name="Table"; t.scale=(1.2,0.8,0.04); mat=bpy.data.materials.new("Wood"); mat.use_nodes=True; b=mat.node_tree.nodes.get("Principled BSDF"); b.inputs["Base Color"].default_value=(0.3,0.18,0.08,1); t.data.materials.append(mat)
for x in [-0.5,0.5]:
 for y in [-0.3,0.3]:
...
print("Table created")"""
            ...
            return "💡 Creating lamp...", """bpy.ops.mesh.primitive_cylinder_add(radius=0.15,depth=0.03,location=(0,0,0.015)); bpy.context.active_object.name="Base"
...
print("Lamp created")"""
        elif "cube" in m: return "📦", "bpy.ops.mesh.primitive_cube_add(size=1,location=(0,0,0.5)); bpy.context.active_object.name='Cube'; print('Cube')"
        elif "sphere" in m: return "🔵", "bpy.ops.mesh.primitive_uv_sphere_add(radius=0.5,location=(0,0,0.5)); bpy.context.active_object.name='Sphere'; print('Sphere')"
        elif "cylinder" in m: return "🔵", "bpy.ops.mesh.primitive_cylinder_add(radius=0.5,depth=1,location=(0,0,0.5)); bpy.context.active_object.name='Cylinder'; print('Cylinder')"
        elif "cone" in m or "cono" in m: return "🔺", "bpy.ops.mesh.primitive_cone_add(radius1=0.5,depth=1,location=(0,0,0.5)); bpy.context.active_object.name='Cone'; print('Cone')"
        elif "torus" in m or "donut" in m: return "🍩", "bpy.ops.mesh.primitive_torus_add(major_radius=0.5,minor_radius=0.2,location=(0,0,0.5)); bpy.context.active_object.name='Torus'; print('Torus')"
        elif "plane" in m or "plano" in m: return "📐", "bpy.ops.mesh.primitive_plane_add(size=2,location=(0,0,0)); bpy.context.active_object.name='Plane'; print('Plane')"
    
    if any(w in m for w in ["delete", "remove", "borrar", "eliminar"]):
        return "🗑️", "bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.delete(); print('Deleted')"
    
    if any(w in m for w in ["color", "paint", "colorear"]):
        c = (0.8,0.8,0.8)
        if "red" in m or "rojo" in m: c=(0.8,0.1,0.1)
        elif "blue" in m or "azul" in m: c=(0.1,0.1,0.8)
        elif "green" in m or "verde" in m: c=(0.1,0.7,0.1)
        elif "yellow" in m or "amarillo" in m: c=(0.9,0.9,0.1)
        elif "black" in m or "negro" in m: c=(0.05,0.05,0.05)
        elif "white" in m or "blanco" in m: c=(0.95,0.95,0.95)
        return "🎨", f"o=bpy.context.active_object\nif o and o.type=='MESH':\n m=o.data.materials[0] if o.data.materials else bpy.data.materials.new('Mat'); o.data.materials.append(m) if not o.data.materials else None\n m.node_tree.nodes.get('Principled BSDF').inputs['Base Color'].default_value=({c[0]},{c[1]},{c[2]},1)\n print('Color applied')\nelse: print('Select mesh')"
    
    if any(w in m for w in ["material", "wood", "madera", "metal", "glass", "vidrio"]):
        p = "wood"
        if "metal" in m: p="metal"
        elif "glass" in m or "vidrio" in m: p="glass"
        pr={"wood":(0.45,0.30,0.15,0.7,0.0),"metal":(0.85,0.65,0.10,0.1,1.0),"glass":(0.95,0.95,0.95,0.0,0.0)}
        v=pr.get(p,pr["wood"])
    ...
    if any(w in m for w in ["undo", "desacer", "volver"]): return "⏪", "bpy.ops.ed.undo(); print('Undo')"
    if any(w in m for w in ["redo", "rehacer"]): return "⏩", "bpy.ops.ed.redo(); print('Redo')"
    
    if "export" in m or "save" in m or "exportar" in m or "guardar" in m or "download" in m or "descargar" in m:
        fmt="glb"
        if "fbx" in m: fmt="fbx"
        elif "obj" in m: fmt="obj"
        elif "stl" in m: fmt="stl"
        return "📤", f"import os; fp=os.path.join(os.path.expanduser('~'),'Desktop',f'export.{{'{fmt}'}}'); bpy.ops.object.select_all(action='SELECT')\ntry:\n if '{fmt}'=='glb': bpy.ops.export_scene.gltf(filepath=fp,export_format='GLB')\n elif '{fmt}'=='fbx': bpy.ops.export_scene.fbx(filepath=fp)\n elif '{fmt}'=='obj': bpy.ops.export_scene.obj(filepath=fp)\n elif '{fmt}'=='stl': bpy.ops.export_mesh.stl(filepath=fp)\n print(f'Exported to {{fp}}')\nexcept Exception as e: print(f'{{e}}')"
    
    if "status" in m or "estado" in m or "info" in m: return "📊", "print(f'Objects: {len(bpy.context.scene.objects)} | Materials: {len(bpy.data.materials)}')"
    if "clean" in m or "limpiar" in m: return "🧹", "bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.delete(); print('Cleaned')"
    if any(w in m for w in ["help", "ayuda", "commands", "comandos"]): return "📚", None
...
classes = (
    AIMessage, AIModelItem, AIProps,
    AI_OT_Send, AI_OT_Clear, AI_OT_AttachImage,
    AI_OT_StartServer, AI_OT_StopServer,
    AI_OT_SaveConfig, AI_OT_TestConnection,
    AI_OT_FetchModels, AI_OT_SelectModel, AI_OT_VoiceToggle,
    AI_PT_Main, AI_PT_Chat, AI_PT_Settings,
)
...
def register():
    ...
    for key in ["provider", "server_url", "selected_model", "temperature", "max_tokens", "language"]:
        if key in config:
