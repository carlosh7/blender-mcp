        ...
        for i, obj_a in enumerate(objs):
            ...
            for obj_b in objs[i + 1 :]:
                try:
                    ...
                    if overlaps:
                        report.append(
                            f"❌ COLISIÓN CRÍTICA: {obj_a.name} y {obj_b.name} se intersectan."
                        )
                    else:
                        ...
                        if dist < 0.0001:
                            report.append(
                                f"⚠️ Z-FIGHTING: {obj_a.name} y {obj_b.name} están solapados (0.00mm)."
                            )
                except Exception:
                    continue
...
def get_spatial_summary():
    ...
    for obj in objs:
        dim = obj.dimensions
        summary.append(
            f"- {obj.name}: {round(dim.x, 3)}x{round(dim.y, 3)}x{round(dim.z, 3)}m en pos {list(map(lambda x: round(x, 3), obj.location))}"
        )
