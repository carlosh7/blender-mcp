...
class GeometryScanner:
    ...
    def get_blueprint(obj):
        ...
        [mw @ v for v in local_coords]

        l_min = Vector(
            (
                min(v.x for v in local_coords),
        ...
        blueprint = {
            ...
            "physics": {"mass_kg": round(float(mass), 2), "ior": round(float(ior), 3)},
            "dimensions": {
    ...
    def detect_holes(obj):
        ...
        return {
            "status": "feature_under_development",
            "message": "Hole detection requires bmesh topology analysis.",
        }
