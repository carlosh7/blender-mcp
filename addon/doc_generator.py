...
def _build_scene_documentation():
    ...
    if cameras:
        ...
        for cam in cameras:
            lines.append(f"\n### {cam.name}")
            lines.append(f"- **Posición**: ({cam.location.x:.2f}, {cam.location.y:.2f}, {cam.location.z:.2f})")
            lines.append(f"- **Rotación**: ({cam.rotation_euler.x:.2f}, {cam.rotation_euler.y:.2f}, {cam.rotation_euler.z:.2f})")
    ...
    if lights:
        ...
        for light in lights:
            data = light.data
            loc = light.location
            lines.append(f"| {light.name} | {data.type} | {data.energy}W | ({loc.x:.1f}, {loc.y:.1f}, {loc.z:.1f}) |")
...
def _build_object_spec(obj):
    ...
    lines.append(f"| Colección | {obj.users_collection[0].name if obj.users_collection else 'Ninguna'} |")
    ...
    lines.append(f"| Centro | ({bb['center'][0]:.4f}, {bb['center'][1]:.4f}, {bb['center'][2]:.4f}) |")
            ...
            if mat.use_nodes:
                for node in mat.node_tree.nodes:
                    if node.type == 'BSDF_PRINCIPLED':
                        ...
                        lines.append(f"- **Color**: ({color[0]:.2f}, {color[1]:.2f}, {color[2]:.2f})")
                        lines.append(f"- **Rugosidad**: {node.inputs['Roughness'].default_value:.2f}")
                        lines.append(f"- **Metalicidad**: {node.inputs['Metallic'].default_value:.2f}")
                        break
    ...
    if children:
        ...
        for child in children:
            ...
            lines.append(f"| {child.name} | {child.type} | ({local_loc.x:.4f}, {local_loc.y:.4f}, {local_loc.z:.4f}) |")
