...
TOOLS = [
    Tool("batch.rename", ToolCategory.BATCH, "Batch rename objects", ToolPermission.WRITE,
         {"pattern": {"type": "str"}, "replace": {"type": "str"}, "type_filter": {"type": "str"}}),
    Tool("batch.delete_by_type", ToolCategory.BATCH, "Delete all objects of a type", ToolPermission.DESTRUCTIVE,
         {"type": {"type": "str", "required": True}}),
    Tool("batch.apply_transforms", ToolCategory.BATCH, "Apply transforms to all objects", ToolPermission.WRITE,
         {"type_filter": {"type": "str"}}),
    Tool("batch.add_modifier", ToolCategory.BATCH, "Add modifier to multiple objects", ToolPermission.WRITE,
         {"object_names": {"type": "list", "required": True}, "modifier_type": {"type": "str", "required": True}}),
    Tool("batch.set_material", ToolCategory.BATCH, "Set material for multiple objects", ToolPermission.WRITE,
         {"object_names": {"type": "list", "required": True}, "material_name": {"type": "str", "required": True}}),
    Tool("batch.turntable", ToolCategory.BATCH, "Create turntable animation", ToolPermission.WRITE,
...
def rename(pattern: str, replace: str, type_filter: str = None) -> Dict:
    try:
        ...
        for obj in bpy.context.scene.objects:
            if type_filter and obj.type != type_filter.upper(): continue
...
def turntable(object_name: str = None, frames: int = 120, axis: str = "Z") -> Dict:
    try:
        ...
        if not obj: return {"error": "No object selected"}
        import math
        bpy.context.scene.frame_start = 1
        bpy.context.scene.frame_end = frames
        for f in range(frames + 1):
            angle = (f / frames) * 2 * math.pi
            if axis.upper() == 'Z': obj.rotation_euler = (0, 0, angle)
            elif axis.upper() == 'Y': obj.rotation_euler = (0, angle, 0)
            else: obj.rotation_euler = (angle, 0, 0)
            obj.keyframe_insert(data_path="rotation_euler", frame=f)
        return {"success": True, "object": obj.name, "frames": frames}
    except Exception as e: return {"error": str(e)}

HANDLERS = {
    "batch.rename": rename, "batch.delete_by_type": delete_by_type,
    "batch.apply_transforms": apply_transforms, "batch.add_modifier": add_modifier_batch,
    "batch.set_material": set_material_batch, "batch.turntable": turntable,
}
