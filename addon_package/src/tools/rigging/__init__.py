...
TOOLS = [
    Tool("rigging.create_armature", ToolCategory.RIGGING, "Create an armature", ToolPermission.WRITE,
         {"name": {"type": "str"}, "location": {"type": "tuple"}}),
    Tool("rigging.add_bone", ToolCategory.RIGGING, "Add a bone to armature", ToolPermission.WRITE,
         {"armature_name": {"type": "str", "required": True}, "name": {"type": "str"},
          "head": {"type": "tuple"}, "tail": {"type": "tuple"}}),
    Tool("rigging.add_constraint", ToolCategory.RIGGING, "Add constraint to bone", ToolPermission.WRITE,
         {"object_name": {"type": "str", "required": True}, "type": {"type": "str", "required": True},
          "target": {"type": "str"}}),
    Tool("rigging.create_vertex_group", ToolCategory.RIGGING, "Create vertex group", ToolPermission.WRITE,
         {"object_name": {"type": "str", "required": True}, "name": {"type": "str"}}),
    Tool("rigging.assign_vertex_group", ToolCategory.RIGGING, "Assign vertices to group", ToolPermission.WRITE,
         {"object_name": {"type": "str", "required": True}, "group_name": {"type": "str", "required": True},
          "weight": {"type": "float"}}),
    Tool("rigging.auto_weight", ToolCategory.RIGGING, "Automatic weight painting", ToolPermission.WRITE,
         {"object_name": {"type": "str"}, "armature_name": {"type": "str"}}),
    Tool("rigging.list_bones", ToolCategory.RIGGING, "List bones in armature", ToolPermission.READ_ONLY,
         {"armature_name": {"type": "str", "required": True}}),
    Tool("rigging.apply_armature", ToolCategory.RIGGING, "Apply armature modifier", ToolPermission.WRITE,
...
def create_armature(name: str = "Armature", location: Tuple = (0, 0, 0)) -> Dict:
    try:
        ...
        if name: arm.name = name
        return {"success": True, "name": arm.name}
    except Exception as e: return {"error": str(e)}

def add_bone(armature_name: str, name: str = "Bone", head: Tuple = (0, 0, 0), tail: Tuple = (0, 0, 1)) -> Dict:
    try:
        import bpy
        arm_obj = bpy.data.objects.get(armature_name)
        if not arm_obj: return {"error": f"Armature not found: {armature_name}"}
...
def add_constraint(object_name: str, type: str, target: str = None) -> Dict:
    try:
        ...
        if target:
            ...
            if target_obj: con.target = target_obj
...
def list_bones(armature_name: str) -> Dict:
    try:
        import bpy
        arm_obj = bpy.data.objects.get(armature_name)
        if not arm_obj or arm_obj.type != 'ARMATURE': return {"error": f"Armature not found: {armature_name}"}
        bones = [{"name": b.name, "head": list(b.head_local), "tail": list(b.tail_local), "parent": b.parent.name if b.parent else None}
...
HANDLERS = {
    "rigging.create_armature": create_armature, "rigging.add_bone": add_bone,
    "rigging.add_constraint": add_constraint, "rigging.create_vertex_group": create_vertex_group,
    "rigging.assign_vertex_group": assign_vertex_group, "rigging.auto_weight": auto_weight,
    "rigging.list_bones": list_bones, "rigging.apply_armature": apply_armature,
}
