...
# Materials Setup
mat_metal_dark = pbr_factory.create_pbr_metal('MAT_Luxo_DarkMetal', color=(0.14, 0.15, 0.17), brushed=True)
mat_metal_chrome = pbr_factory.create_pbr_metal('MAT_Luxo_Chrome', color=(0.85, 0.86, 0.88), brushed=True)
mat_shade_white = pbr_factory.create_pbr_metal('MAT_Luxo_WhiteEnamel', color=(0.92, 0.92, 0.94), brushed=False)
mat_spring_steel = pbr_factory.create_pbr_metal('MAT_Luxo_SpringSteel', color=(0.6, 0.62, 0.65), brushed=True)
...
base_profile = [
    (0.001, 0.0), (0.16, 0.0), (0.175, 0.008), (0.17, 0.02),
    (0.14, 0.032), (0.05, 0.045), (0.035, 0.06), (0.001, 0.06)
]
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.025, depth=0.05, location=(0, 0, 0.075), rotation=(math.pi/2, 0, 0))
...
for i, offset_y in enumerate([-0.025, 0.025]):
    bpy.ops.mesh.primitive_cylinder_add(radius=0.005, depth=l1, location=(0, offset_y, 0.075 + l1/2.0))
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.025, depth=0.05, location=(elbow_x, 0, elbow_z), rotation=(math.pi/2, 0, 0))
...
for i, offset_y in enumerate([-0.020, 0.020]):
    bpy.ops.mesh.primitive_cylinder_add(radius=0.005, depth=l2, location=(elbow_x, offset_y, elbow_z))
    arm = bpy.context.active_object
...
bpy.ops.mesh.primitive_cylinder_add(radius=0.022, depth=0.045, location=(head_x, 0, head_z), rotation=(math.pi/2, 0, 0))
...
shade_profile = [
    (0.025, 0.0), (0.04, 0.03), (0.075, 0.08), (0.13, 0.15), (0.165, 0.19),
    (0.175, 0.205), (0.17, 0.21), (0.155, 0.20), (0.12, 0.145), (0.07, 0.075),
    (0.035, 0.028), (0.02, 0.0)
]
